...
/* GPIOs */
#define CC1120_CS_PORT 		GPIOA
#define CC1120_CS_PIN 		GPIO_PIN_4
#define CC1120_MISO_PORT	GPIOA
#define CC1120_MISO_PIN		GPIO_PIN_6
#define CC1120_RESETN_PORT	GPIOA
#define CC1120_RESETN_PIN	GPIO_PIN_8

/*
 * Prototypes
 */
static uint8_t trx8BitRegAccess(uint8_t accessType, uint8_t addrByte, uint8_t *pData, uint16_t len, rfStatus_t *status);
static uint8_t trx16BitRegAccess(uint8_t accessType, uint8_t extAddr, uint8_t regAddr, uint8_t *pData, uint8_t len, rfStatus_t *status);
static uint8_t CC1120_SPI_Begin();
static void CC1120_SPI_End();
static uint8_t trxReadWriteBurstSingle(uint8_t addr,uint8_t *pData, uint16_t len);



void cc1120_SPI_Init(SPI_HandleTypeDef *spi)
{
	/* Init global spi peripheral handle */
	hspi = spi;
}

void cc1120_SPI_Deinit(void)
{
	hspi = NULL;
}

void cc1120_GpioReset(void)
{
    /* CS high */
    HAL_GPIO_WritePin(CC1120_CS_PORT, CC1120_CS_PIN, GPIO_PIN_SET);
    HAL_Delay(50);

    /* Hard reset */	
    HAL_GPIO_WritePin(CC1120_RESETN_PORT, CC1120_RESETN_PIN, GPIO_PIN_RESET);
	HAL_Delay(50);
	HAL_GPIO_WritePin(CC1120_RESETN_PORT, CC1120_RESETN_PIN, GPIO_PIN_SET);
    HAL_Delay(50);
}
 ...
 * @return      rfStatus_t
 */
uint8_t cc112xSpiReadReg(uint16_t addr, uint8_t *pData, uint8_t len, rfStatus_t *status)
{
  ...
  if(!tempExt)
  {
    retval = trx8BitRegAccess((RADIO_BURST_ACCESS|RADIO_READ_ACCESS),tempAddr,pData,len, status);
  }
  else if (tempExt == 0x2F)
  {
    retval = trx16BitRegAccess((RADIO_BURST_ACCESS|RADIO_READ_ACCESS),tempExt,tempAddr,pData,len, status);
  }
  return retval;
}
 ...
 * @return      rfStatus_t
 */
uint8_t cc112xSpiWriteReg(uint16_t addr, uint8_t *pData, uint8_t len, rfStatus_t *status)
{
  uint8_t tempExt  = (uint8_t)(addr>>8);
  uint8_t tempAddr = (uint8_t)(addr & 0x00FF);
  uint8_t retval = 0;
  ...
  if(!tempExt)
  {
    retval = trx8BitRegAccess((RADIO_BURST_ACCESS|RADIO_WRITE_ACCESS),tempAddr,pData,len, status);
  }
  else if (tempExt == 0x2F)
  {
    retval = trx16BitRegAccess((RADIO_BURST_ACCESS|RADIO_WRITE_ACCESS),tempExt,tempAddr,pData,len, status);
  }
  return retval;
}
 ...
 * @return      error code
 */
uint8_t cc112xSpiWriteTxFifo(uint8_t *pData, uint8_t len, rfStatus_t *status)
{
  return trx8BitRegAccess(0x00, CC112X_BURST_TXFIFO, pData, len, status);
}
 ...
 * @return      rfStatus_t
 */
uint8_t cc112xSpiReadRxFifo(uint8_t * pData, uint8_t len, rfStatus_t *status)
{
  return trx8BitRegAccess(0x00, CC112X_BURST_RXFIFO, pData, len, status);
}
 ...
 *
 */
uint8_t cc112xGetTxStatus(rfStatus_t * status)
{
    return trxSpiCmdStrobe(CC112X_SNOP, status);
}
 ...
 *
 */
uint8_t cc112xGetRxStatus(rfStatus_t *status)
{
    return trxSpiCmdStrobe((CC112X_SNOP | RADIO_READ_ACCESS), status);
}
...
static uint8_t trx8BitRegAccess(uint8_t accessType, uint8_t addrByte, uint8_t *pData, uint16_t len, rfStatus_t *status)
{
  ...
  retval = CC1120_SPI_Begin();	  
  if (retval) {
    return CC1120_ERR_SPI_BEGIN;
  }
...
static uint8_t trx16BitRegAccess(uint8_t accessType, uint8_t extAddr, uint8_t regAddr, uint8_t *pData, uint8_t len, rfStatus_t *status)
{
  ...
  /* and read back chip status */
  txData = accessType | extAddr;
  retval = HAL_SPI_TransmitReceive(hspi, &txData, status, 1, 1000);  
  if (retval) {
    CC1120_SPI_End();
    return CC1120_ERR_SPI_XFER;
  ...
  retval = HAL_SPI_Transmit(hspi, &regAddr, 1, 1000);
  if (retval) {
    CC1120_SPI_End();
    return CC1120_ERR_SPI_XFER;
  }
  ...
  retval = trxReadWriteBurstSingle(accessType|extAddr,pData,len);
  if (retval) {
    CC1120_SPI_End();
    return CC1120_ERR_SPI_XFER;
  }
...
static uint8_t CC1120_SPI_Begin()
{
	...
	/* CS low */
	HAL_GPIO_WritePin(CC1120_CS_PORT, CC1120_CS_PIN, GPIO_PIN_RESET);
	
    /* Wait MISO low */
	while (HAL_GPIO_ReadPin(CC1120_MISO_PORT, CC1120_MISO_PIN) == GPIO_PIN_SET) {
